...
main() {
  ...
  *)
  ...
  # Status left
  if $show_powerline; then
    tmux set-option -g status-left "#[bg=${cyan},fg=${dark_gray}]#{?client_prefix,#[bg=${yellow}],} ${left_icon} #[fg=${cyan},bg=${dark_gray}]#{?client_prefix,#[fg=${yellow}],}${left_sep}"
    powerbg=${dark_gray}
  else
  ...
  for plugin in "${plugins[@]}"; do
    ...
    if case $plugin in custom:*) true ;; *) false ;; esac then
      ...
      if [ -n "$time_format" ]; then
        ...
        _timezone=""
        [ -n "$timezone" ] && _timezone=" $timezone"

        clock_icons=("󱑋" "󱑌" "󱑍" "󱑎" "󱑏" "󱑐" "󱑑" "󱑒" "󱑓" "󱑔" "󱑕" "󱑖")
        clock_icon=${clock_icons[$(date +%H) | bc]}
        clock_icon=" $clock_icon"

        if $show_day_month && $show_military; then # military time and dd/mm
          script="%a %d/%m %R${_timezone}$clock_icon"
        elif $show_military; then # only military time
          script="%a %m/%d %R${_timezone}$clock_icon"
        elif $show_day_month; then # only dd/mm
          script="%a %d/%m %I:%M %p${_timezone}$clock_icon"
        else
          script="%a %m/%d %I:%M %p${_timezone}$clock_icon"
        fi
      fi
